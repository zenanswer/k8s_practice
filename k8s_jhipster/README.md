...
```
$ kubectl scale deployment <app-name> --replicas <replica-count> -n tag
```
...
```
$ kubectl set image deployment/<app-name>-app <app-name>=<new-image>  -n tag
```
...
```
$ kubectl get svc jhipster-console -n tag
```
...
```
$ kubectl expose service jhipster-registry --type=NodePort --name=exposed-registry -n tag
```

and explore the details using

```
$ kubectl get svc exposed-registry -n tag
```

For scaling the JHipster registry, use

```
$ kubectl scale statefulset jhipster-registry --replicas 3 -n tag
```
